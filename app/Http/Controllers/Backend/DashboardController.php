...
class DashboardController extends Controller
{
    public function getBookingChartData(Request $request)
    {
        ...
        $offset = intval($request->query('offset', 0)); // thêm offset
        $today = Carbon::now();
        ...
        if ($type === 'week') {
            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                $total = Booking::whereDate('check_in', $day)->sum('total_price');
                $data[] = [
            ...
            for ($day = $start; $day->lte($end); $day->addDays(2)) {
                ...
                $total = Booking::whereBetween('check_in', [$day, $next])->sum('total_price');
                $data[] = [
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                $total = Booking::whereBetween('check_in', [$monthStart, $monthEnd])->sum('total_price');
                $data[] = [
